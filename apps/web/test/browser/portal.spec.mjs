...
const currentReleaseDoi = "10.5281/zenodo.21315776";
const previousReleaseDoi = "10.5281/zenodo.21285595";
const earlierReleaseDoi = "10.5281/zenodo.21142690";
...
async function expectCitationMetadata(page) {
  ...
  await expect(page.locator("meta[name='citation_publication_date']")).toHaveAttribute("content", "2026-07-11");
  await expect(page.locator("meta[name='citation_doi']")).toHaveAttribute("content", currentReleaseDoi);
  await expect(page.locator("meta[name='citation_software_version']")).toHaveAttribute("content", "0.1.4-research-alpha");
...
test("publishes SEO, social preview, and structured metadata", async ({ page, request }) => {
  ...
  expect(softwareMetadata.version).toBe("0.1.4-research-alpha");
  expect(softwareMetadata.sameAs).toContain(`https://doi.org/${currentReleaseDoi}`);
  expect(softwareMetadata.sameAs).toContain("https://zenodo.org/records/21315776");
  ...
  expect(softwareMetadata.identifier).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ propertyID: "Zenodo release DOI", value: currentReleaseDoi, url: `https://doi.org/${currentReleaseDoi}` }),
  ...
  await expect(page.getByText("Cite v0.1.4:")).toBeVisible();
...
test("serves public resource pages with canonical metadata and notices", async ({ page }) => {
  ...
  await expect(page.getByText("Use DOI 10.5281/zenodo.21315776 when citing v0.1.4-research-alpha")).toBeVisible();
  await expect(page.getByRole("link", { name: currentReleaseDoi })).toHaveAttribute("href", `https://doi.org/${currentReleaseDoi}`);
  await expect(page.getByRole("link", { name: previousReleaseDoi })).toHaveAttribute("href", `https://doi.org/${previousReleaseDoi}`);
  await expect(page.getByRole("link", { name: earlierReleaseDoi })).toHaveAttribute("href", `https://doi.org/${earlierReleaseDoi}`);
  ...
  await expectCitationMetadata(page);
  const v014Release = page.locator(".portal-page-section").filter({ has: page.getByRole("heading", { name: "v0.1.4 Research Alpha" }) });
  await expect(v014Release).toBeVisible();
  await expect(v014Release.getByText("Zenodo DOI status: minted.")).toBeVisible();
  await expect(v014Release.getByRole("link", { name: "Release notes" })).toHaveAttribute("href", /v0\.1\.4-research-alpha\.md/);
  await expect(v014Release.getByRole("link", { name: "Validation report JSON" })).toHaveAttribute("href", /v0\.1\.4-research-alpha-validation-report\.json/);
  await expect(v014Release.getByRole("link", { name: "SBOM JSON" })).toHaveAttribute("href", /v0\.1\.4-research-alpha-sbom\.json/);
  await expect(v014Release.getByRole("link", { name: "Zenodo DOI" })).toHaveAttribute("href", `https://doi.org/${currentReleaseDoi}`);
  await expect(v014Release.getByRole("link", { name: "Zenodo all-versions DOI" })).toHaveAttribute("href", `https://doi.org/${allVersionsDoi}`);
  ...
  await expect(v013Release.getByRole("link", { name: "Zenodo DOI" })).toHaveAttribute("href", `https://doi.org/${previousReleaseDoi}`);
  ...
  await expect(v012Release.getByRole("link", { name: "Zenodo DOI" })).toHaveAttribute("href", `https://doi.org/${earlierReleaseDoi}`);
