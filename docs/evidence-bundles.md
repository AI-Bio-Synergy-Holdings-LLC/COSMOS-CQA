...
Pinned observation evidence is visualized as a 3x3 zone map and a derived summary by tile, zone, row band, column band, radial band, class, severity, review status, consensus placeholder status, and QA metric rows. These summaries show review attention patterns and workflow state only. `needs-adjudication` is a review placeholder, not scientific consensus. The interpretation boundary is documented in `docs/tile-observation-notes.md`.
