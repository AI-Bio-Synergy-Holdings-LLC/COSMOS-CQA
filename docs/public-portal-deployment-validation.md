...
- `apps/web/partner-readiness.html` distinguishes the public local-first demo from the future selective-access application without access promises, partnership acceptance claims, deployment-specific backend internals, operational timelines, data-use agreements, confidentiality offers, support obligations, or production capability statements;
- `apps/web/docs.html` and `apps/web/partner-readiness.html` link the control-level private-application transition map and readiness gates without exposing private repository locations, credentials, provider configuration, partner decisions, or security findings;
...
- `.zenodo.json`, `CITATION.cff`, citation docs, release pages, README, portal structured data, citation meta tags, and visible DOI affordances preserve controlled Zenodo DOI metadata, DOI-minted release wording, and the research-only license boundary;
...
- transition-planning links remain control-level and do not publish deployment-specific private implementation details.
